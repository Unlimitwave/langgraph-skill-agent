...
1. **本机 Python（推荐）**：使用 `workspace_exec_python`，`program` 为 `python3`，`argv_tail` 为 `["skills/test-calc-script/run_calc.py"]`。
2. **白名单 shell**：使用工具 `run_skill_script_shell`，`script_id` 为 `test-calc.run`（执行 `skills/test-calc-script/run_calc.sh`）。
