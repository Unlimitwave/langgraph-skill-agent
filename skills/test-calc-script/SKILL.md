...
1. **优先（Docker 隔离）**：使用工具 `run_skill_script_in_docker`，参数为 `test-calc-script/run_calc.py`（相对 `skills/`），在只读挂载的隔离容器中执行。
2. **白名单 shell**：使用工具 `run_skill_script`，`script_id` 为 `test-calc.run`（执行 `skills/test-calc-script/run_calc.sh`）。
3. **本机 Python**：使用 `workspace_exec`，`program` 为 `python3`，`argv_tail` 为 `["skills/test-calc-script/run_calc.py"]`。
